...
Resolved later the same day — see "Same-day close-out round".
...
## Same-day close-out round

The user confirmed the four agent presets have no retention need, so the preset breakage above was resolved by removal rather than by schema migration.

- Backed up all four preset directories (27 files, 309 KB, sha256 of every `*.yml` recorded) to `.local/dsh-rollback-20260915/preset-backup-20260915/`, then deleted `~\.dsh\.agent-presets\{apex-standard,anchored-standard,pristine,liangshen}`.
- Deleting `liangshen` alone did not stick: `@linxin666/dsh-liangshen` calls `syncPresetTrees`/`syncOnePreset` and re-creates `~/.dsh/.agent-presets/liangshen` on every host start (observed directly — the directory reappeared after the next boot). Added a **fourteenth** quarantine row, `web-ui-liangshen`, in its own commented block because the reason differs from the thirteen incompatibility quarantines. `~\.dsh\.agent-presets` is now empty and stays empty across boots.
- Removed the `agent-presets` override from `settings.yaml` instead of naming a replacement: the shipped `@deepseek-ai/dsh-web-app` layer already composes `agent-presets.config.default: standard`, so `--dump-config` now resolves `default: standard` with no user layer. `agent-default-model` (tokenrhythm / glm-5.3-flash) untouched.
- `@mlgbnb/dsh-archive-manager` **kept, not quarantined**. Evidence: its published `package.json` declares `dependencies: {}` (only a `react` peer); zero `@deepseek-ai/` specifiers occur anywhere in `lib/` (its client half imports only `react`, its host half only `node:fs/os/path/zlib`). Its stale `dsh.client.inject` list still names `dsh-client-runtime` and `dsh-client-ui-slots`, both gone from the product and unresolvable through the link farm too, yet the plugin mounts, appears in `window.__DSH_BOOT__`, renders its 归档管理 settings entry, and produces no "missed the module table" error — unlike `dsh-perf`/`aionui-panel`, which do. So it does not depend on the old symlink farm, and no implicit dependency needed removing.
- Post-cleanup boot is fully clean: 82-byte log (URL line only), zero error lines across the whole session, no `$.prefix missing required value`. New session creation works, the composer shows 标准模式, and the session projection records `provider=tokenrhythm`, `model=deepseek-flash`.
- DSH-internal smoke: **text PASS** ("Reply with exactly the word OK" → `OK`); **tool call PASS** (read `package.json` → correctly answered `@deepseek-ai/cordis`, shown as "1 次工具调用"). **Vision not completed inside DSH**: the Web attachment validator rejects programmatically constructed `File` objects (correct PNG magic, `type: image/png`, `.png` name) with "仅支持 PNG、JPG、WebP、GIF 格式的图片" at submit time even though it previews the thumbnail, and this in-app browser exposes no real file chooser or trusted drop. This is a harness limitation, not a model/route/config finding; the identical payload passed vision over the direct `tokenrhythm` route.
- Side effect worth knowing: this verification created one real DSH session under the `D:\DSH` workspace ("Requesting exact OK reply only"). It was left in place rather than deleted.
