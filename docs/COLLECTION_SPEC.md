...
- project lifecycle 的 `status`：非强制字段，项目存在本身即表达其长期项目地位；确需记录时只能使用 lifecycle 集合（见下文 curated 小节：active / paused / maintenance / archived / experimental）。软件的 exceptional status（legacy / testing / broken / inactive）是另一套语义，不要混用到 project 上；
