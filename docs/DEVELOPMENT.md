...
- `scripts/curate.ps1` 已提供 confirmation-manifest 基础设施：默认只生成 dry-run plan，只有显式 `-Apply` 才能更新现有实体的 allowlisted `curated` 字段或已确认 conventions；manifest 中出现 `observed`、未知 stable ID、空更新集或缺少 evidence/confirmation 时直接拒绝。当前没有任何确认 manifest 被应用。
